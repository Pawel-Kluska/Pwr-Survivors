...
public class PlayerInventory : MonoBehaviour
{
    ...
    void ApplyUpgradeOptions(bool evolution = false)
    {
        ...
        List<ItemData> availableEvolutions = new List<ItemData>();
        ...
        if (evolution)
        {
            foreach (Slot s in weaponSlots)
            {
                Item w = s.item;
                if (!w) continue;
                if (w.data.evolutionData.Length == 0) continue;

                if (w.CanEvolve(w.data.evolutionData[0], 0))
                {
                    //if (e.condition == ItemData.Evolution.Condition.treasureChest)
                    availableEvolutions.Add(w.data);
                }
            }
        }
        ...
        if (availUpgradeCount > 0)
            upgradeWindow.SetUpgrades(this, availableEvolutions, availableUpgrades, 4);
    ...
    public void RemoveAndApplyUpgrades(bool evolution = false)
    {
        ApplyUpgradeOptions(evolution);
    }
